...
class StudentServiceTest {
    ...
    void findStudent_success() {
        long id = 1;
        Student harry = new Student(id, "Harry Potter", 11);

        when(studentRepository.findById(id)).thenReturn(Optional.of(harry));

        Optional<Student> optionalResult = studentService.findStudent(id);

        assertEquals(harry, optionalResult.orElse(null));
    }
